...
_autodir() {
    local cur
    cur=${COMP_WORDS[*]:1}
    comps=$(cmd_client.py text_search dir num $cur |pec)
    while read i; do
...
_autocmd() {
        local cur
        cur=${COMP_WORDS[*]:1}
        comps=$(cmd_client.py text_search cmds num $cur |pec)
        while read -r i; do
            COMPREPLY=("${COMPREPLY[@]}" "${i}")
        done <<EOF
        $comps
...
_autofile() {
        local cur
        cur=${COMP_WORDS[*]:1}
        comps=$(cmd_client.py text_search file num $cur| pec)
        while read i; do
            COMPREPLY=("${COMPREPLY[@]}" "${i}")
        done <<EOF
        $comps
...
sgc() {
    string="${@}"
    cmd_client.py text_search cmds num $string |pec
}

sgd() {
    string="${@}"
    cmd_client.py text_search dir num $string |pec
}

sgf() {
    string="${@}"
    cmd_client.py text_search file num $string |pec
}
...
bash_prompt_client() {
    ...
    if [ "$msgs" != "$PRE_CMD" ]; then
        cmd_client.py auto_cmd $PWD $msgs
        PRE_CMD=$msgs
    fi
}
